 ...
 */
export function resolveOrders(gameState, allOrders) {
    const log = [];
    const rs = {
        orders: new Map(),
        units: new Map(),
...
function validateOrders(rs, log) {
    for (const [province, order] of rs.orders) {
...
function canUnitReach(unit, target) {
...
function getAttackStrength(province, destination, rs) {
    let strength = 1;
    for (const [supProvince, supOrder] of rs.orders) {
        if (supOrder.type !== 'support')
            continue;
        if (rs.cutSupports.has(supProvince))
            continue;
        const support = supOrder;
        if (support.supportedProvince === province &&
...
function getDefendStrength(province, rs) {
...
function getHoldStrength(province, rs) {
    if (!rs.units.has(province))
        return 0;
    const order = rs.orders.get(province);
    if (!order)
        return 0;
...
function resolveMoves(rs, log) {
    ...
    for (const [dest, attackers] of movesByDest) {
        ...
        const secondStrength = attacks.length > 1 ? attacks[1].strength : 0;
        ...
        if (defenderOrder?.type === 'move' &&
            defenderOrder.destination === strongest.province) {
            headToHead = true;
        }
        if (attacks.length === 1) {
            ...
            if (headToHead) {
                ...
                const defAttackStr = getAttackStrength(dest, strongest.province, rs);
                if (strongest.strength > defAttackStr &&
                ...
                if (strongest.strength > holdStrength) {
                    ...
                    markMoveFailed(strongest.province, rs, log, 'Not enough strength');
                }
            }
            else {
                ...
                if (strongest.strength > holdStrength) {
                    ...
                    markMoveFailed(strongest.province, rs, log, `Held (attack ${strongest.strength} vs defense ${holdStrength})`);
                }
            }
        }
        else {
            ...
            if (strongest.strength > secondStrength && strongest.strength > holdStrength) {
                ...
                for (const attack of attacks) {
                    markMoveFailed(attack.province, rs, log, `Standoff at ${dest} (strength ${attack.strength})`);
                }
            }
        }
    }
...
function resolveChainedMoves(rs, _chainLog) {
...
function markDislodged(province, from, rs) {
    rs.dislodged.set(province, from);
}
function determineDislodged(rs, _gameState) {
    const retreats = [];
...
function wasStandoffAt(province, rs) {
    let failedMoves = 0;
 ...
 */
export function applyResults(gameState, results, dislodged) {
    ...
    gameState.units = gameState.units.filter((u) => !dislodgedProvinces.has(u.province));
}
