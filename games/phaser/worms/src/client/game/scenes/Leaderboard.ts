...
export class Leaderboard extends Scene {
  ...
  private renderLeaderboard(entries: PlayerStats[]): void {
    ...
    const colX = {
      rank: cx - tableW / 2 + 30,
      name: cx - tableW / 2 + 60,
      wins: cx + tableW / 2 - 120,
      losses: cx + tableW / 2 - 60,
    ...
    this.add.text(colX.losses, startY + rowH / 2, 'L', headerStyle).setOrigin(0.5);
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      this.add.text(colX.losses, y + rowH / 2, `${entry.losses}`, {
        fontFamily: 'monospace',
        fontSize: '11px',
        color: '#e94560',
      }).setOrigin(0.5);
