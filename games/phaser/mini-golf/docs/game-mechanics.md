...
Two-lane zigzag with a single horizontal divider at y=420 creating a bottom lane and a top lane. The divider has a wide 150px gap on the right side (x:300→450). The tee is at x:100,y:650 (bottom-left) and the cup at x:400,y:200 (top-right). The player must send the ball RIGHT across the bottom lane, through the gap, then LEFT across the top lane to the cup. A 45-degree chocolate block deflector at x:420,y:380 near the gap helps redirect the ball upward. Two ice cream slick zones (using `ICE_FRICTION_AIR` 0.012) cover the center portions of each lane — the ball glides across these low-friction surfaces with minimal momentum loss. The right side of the course and the area around the cup are large friction zones (sand physics) where the ball decelerates to a controllable speed. The player must manage power carefully: too much sends the ball bouncing wildly on ice, too little leaves it stranded. Gentle, well-aimed taps let the ball glide through the geometric path. This hole inverts the game's friction paradigm — instead of fighting friction, the player masters a slippery environment where aim angle and power restraint are paramount.
