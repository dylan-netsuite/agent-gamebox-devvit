...
Zigzag stepping-stone layout with candy-cane walled platforms over a dark void. Three platforms connected by offset bridges that force doglegs — no straight line from tee to cup. Bottom platform (150-350, 600-700) with tee at (250,670), LEFT bridge (160-220, 460-600) connects to the wider middle staging platform (120-380, 340-460), then a RIGHT bridge (300-360, 220-340) connects to the cup island (140-380, 150-220) with cup at LEFT (190,185). Each platform's walls have gaps at bridge connection points, allowing the ball to pass through freely. **Gumdrop bumpers** are placed at each switchback corner (bottom-left bridge entry, bottom-left middle platform, top-right middle platform, bottom-right cup island) to help guide the ball through the doglegs. An overhead **claw** patrols the middle platform in a fast figure-eight (lemniscate) pattern centered at (250,400) with amplitude 160x130. The claw casts a 45-unit-radius shadow (with reddish tint) on the ground. The claw checks for ball intersection at ALL times — not just during ball simulation — making it a constant threat even while the player lines up their shot. If the ball enters the shadow zone, the claw grabs it — freezing the ball, playing a grab animation (1.2s), then triggering a +1 penalty and tee reset. An 800ms grace period after each shot prevents immediate grabs. Strategy requires navigating two doglegs while timing shots to avoid the fast-moving claw shadow on the middle platform.
...
| Grab Animation | 1.2 seconds (claw closes, lifts, penalty applied) |
