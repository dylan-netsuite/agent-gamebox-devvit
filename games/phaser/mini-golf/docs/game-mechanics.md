...
Long straightaway into a simulated 360° loop. Tee at (250,720) bottom center, cup at (250,120) top center. The corridor (x:190-310, 120 units wide) runs continuously from the tee through the green, widening into a T-shape at the top (x:150-350, y:75-210). A slick zone (ice friction 0.012) covers the lower corridor (y:550-730) to help the ball maintain speed. At y:450, a loop trigger zone checks the ball's velocity. If the ball exceeds the minimum speed threshold (~2.5 design units scaled), it enters a 360° tween animation: the ball enters from the bottom of the loop circle, sweeps clockwise (right side up, over the top, down the left side), and exits above the loop with upward velocity. Depth-based scaling creates a 3D perspective effect: the ball appears large (1.0x) at the bottom/foreground and small (0.4x) at the top/background. Two-layer rendering draws the back half of the loop behind the ball and the front half in front, creating proper 3D occlusion. The animation takes ~900ms. On exit, the ball is placed above the loop top with ~55% of its entry speed, continuing upward through the corridor into the green. If the ball enters the trigger too slowly, it is rejected backward. Water hazards flank the green on the left (x:80-148) and right (x:352-420), and a water zone behind the green (x:120-380, y:20-75) punishes overshoot. A friction zone on the green (x:170-330, y:100-180) helps the ball stop near the cup. The loop graphic is rendered as a metallic silver circle with guide rails at the entry/exit and a gold directional arrow at the top, inspired by real mini-golf metal loop-de-loops. **Strategy**: 30%+ power reaches the loop; 70%+ power gives a hole-in-one; below 30% the ball doesn't reach the loop trigger zone.
...
| Animation | 360° CW sweep from bottom (PI/2) through full circle, 900ms |
| Depth Scale | 0.4x at top (background), 1.0x at bottom (foreground) |
| Exit Position | Above loop top (cy - radius - 5px), prevents re-trigger |
| Exit Velocity | 55% of entry speed, directed upward |
| Rejection | Velocity reversed at 40% horizontal, 60% vertical bounce |
| Exit Grace | 50ms post-exit immunity from zone effects |
| Rendering | Two layers: back half at depth 5, front half at depth 12 |
| Visual | Metallic silver circle, guide rails, rivets, gold directional arrow |
