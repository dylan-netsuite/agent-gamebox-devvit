...
router.get('/api/stats', async (_req, res): Promise<void> => {
  try {
    ...
    if (!userId) {
      res.status(401).json({ success: false, error: 'Not authenticated' } satisfies StatsResponse);
      return;
    }
    const stats = await getUserStats(userId);
    res.json({ success: true, stats } satisfies StatsResponse);
  } catch (error) {
    console.error('Error in GET /api/stats:', error);
    res.status(500).json({ success: false, error: 'Internal server error' } satisfies StatsResponse);
  }
});
...
router.post('/api/score/submit', async (req, res): Promise<void> => {
  try {
    ...
    if (!userId) {
      res
        .status(401)
        .json({ success: false, error: 'Not authenticated' } satisfies SubmitScoreResponse);
      return;
    }
    ...
    stats.totalDistance += body.distance;
    ...
    await redis.zAdd('dino:lb:top', { member: userId, score: stats.highScore });

    const username = context.username;
    if (username) {
      await redis.hSet('dino:lb:names', { [userId]: username });
    }

    res.json({ success: true, isHighScore } satisfies SubmitScoreResponse);
  } catch (error) {
    console.error('Error in POST /api/score/submit:', error);
    res
      .status(500)
      .json({ success: false, error: 'Internal server error' } satisfies SubmitScoreResponse);
  }
});

async function getUsername(userId: string): Promise<string> {
  try {
    const name = await redis.hGet('dino:lb:names', userId);
    if (name) return name;
  } catch {
    /* ignore */
  }
...
router.get('/api/leaderboard', async (_req, res): Promise<void> => {
  try {
    const topMembers = await redis.zRange('dino:lb:top', 0, 19, { by: 'rank', reverse: true });
    ...
    if (userId) {
      try {
        const rank = await redis.zRank('dino:lb:top', userId);
