...
Private playtest users can choose **Settings → Start fresh**. `POST /api/sandy-shore-pairs-v1/reset` requires a signed-in user and the trusted subreddit name `crossworld_game_dev`; it accepts the expected current `run` and cannot target another account. The response replaces the map with an unrevealed first pair. Public/other installations do not expose this control and reject the route. A confirmation precedes the request, and resets have a 45-second cooldown.

Restart rotates the caller's progress namespace through `cq:sandy-shore-pairs-v1:<user>:run`. Initial boards preserve the original keys and omit `run` in their read response; restarted boards expose an opaque run identifier and use it in draft and submit requests. Progress keys then include `:run:<id>`. Older requests default to the initial run and receive a conflict after reset. In-flight reviews write only their original run, so they cannot restore a restarted board. Verdict caches, review cooldowns and daily budgets remain shared across restarts. Previous run records and older Chatterbloom scenarios stay stored; this is a gameplay restart, not account-data deletion.
...
The restart revision passes the shared gate with **71 tests**, including private access, account isolation, stale clients, concurrent reset clicks, in-flight reviews and preserved paid-call protections. Ten isolated browser checks cover cancellation, reset failure, completed-to-empty transition, reload, fresh-run draft/submission, another-tab restart and unavailable reset controls.

On private build **0.0.9.2**, the user-authorized reset of BarryBetsALot's completed board returned HTTP 200. A full Reddit reopen preserved the fresh run: zero completed pairs, zero revealed tiles, five cover groups, empty words/clues/criteria and an enabled first discovery control. The confirmation uses an in-game HTML dialog because the observed Reddit iframe sandbox does not allow native browser modal dialogs. No words were entered after the reset; suitegeek's state was not reset.
