...
export function createTurnGame(
  ...
  async function saveDraft(user: string, raw: unknown): Promise<Turn> {
  ...
  async function submit(
    ...
    const { issues, word, clue } = validate(parsed);
    ...
    if (completed)
      return finish(user, draft, JSON.parse(completed) as Judgment);
